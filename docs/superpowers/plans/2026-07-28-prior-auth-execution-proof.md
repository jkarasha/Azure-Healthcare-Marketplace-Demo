...
**Tech Stack:** Python 3.11, pytest, ruff, Microsoft Agent Framework (`agent_framework`, `agent_framework_orchestrations`), Make, GitHub Actions.
...
- No live LLM calls. No running MCP servers. Every test added here must pass on a clean checkout with only `requirements-dev.txt` installed.
...
**Create:**
- `requirements-dev.txt` — root dev/test dependencies (pytest only; nothing that pulls Azure SDKs)
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest --collect-only -q 2>&1 | tail -5
```
Expected: `No module named pytest`. This confirms the starting state.
...
```
# Developer / CI test dependencies for the repo-root pytest suite.
# Deliberately minimal: the root unit suite must install and run without
...
`httpx` is included because `tests/conftest.py` imports it at module scope for the MCP integration fixtures; without it, collection of the whole `tests/` tree fails even for unit tests.

- [ ] **Step 3: Install it**

Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pip install -r requirements-dev.txt
```
Expected: ends with `Successfully installed ...` including `pytest`.

- [ ] **Step 4: Confirm collection is still broken for the right reason**
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/eval -q
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest -q -m "not integration" 2>&1 | tail -5
```
...
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo
git add requirements-dev.txt pyproject.toml
git commit -m "test: make the root pytest suite runnable

Add requirements-dev.txt and stop collecting tests from src/, which
contains vendored Azure Functions dependencies rather than first-party
tests. The eval suite has never been executable in this repo.
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && PYTHONPATH=src python3 -m pytest tests/unit/test_parsing.py -q 2>&1 | tail -5
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_parsing.py -q 2>&1 | tail -3
```
Expected: `14 passed`.
...
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo
python3 -m ruff check src/agents/workflows/parsing.py tests/unit/
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_parsing.py -q 2>&1 | tail -5
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m ruff check src/agents/workflows/ tests/unit/ && python3 -m pytest tests/unit -q 2>&1 | tail -3
```
Expected: ruff reports `All checks passed!` and pytest reports `20 passed`.

> Note: `python3 -c "import agents.workflows.prior_auth"` will fail without the agent venv — that is expected and not a regression. Import-check the workflow with the venv instead:
> ```bash
> cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo/src && ./agents/.venv/bin/python -c "import agents.workflows.prior_auth; print('ok')"
> ```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_assessment_schema.py -q 2>&1 | tail -5
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_assessment_schema.py -q 2>&1 | tail -3
```
Expected: `13 passed`.
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 scripts/eval_prior_auth.py --runs-dir data/cases 2>&1 | tail -20
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/eval tests/unit -q 2>&1 | tail -3
```
...
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo
python3 -m ruff check src/agents/workflows/assessment_schema.py tests/ scripts/eval_prior_auth.py
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_golden_case.py -q 2>&1 | tail -20
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_golden_case.py -q 2>&1 | tail -5
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 scripts/eval_prior_auth.py --runs-dir data/cases 2>&1 | tail -30
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_sample_inputs.py -q 2>&1 | tail -5
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_sample_inputs.py -q 2>&1 | tail -3
```
Expected: `4 passed`.
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo/src && ./agents/.venv/bin/python -c "
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_agent_instructions.py -q 2>&1 | tail -10
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -m pytest tests/unit/test_agent_instructions.py -q 2>&1 | tail -3
```
Expected: `3 passed`.
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && grep -rn "never DENY\|only recommend\|Never Recommends DENY" --include="*.py" --include="*.md" . | grep -v node_modules | grep -v .python_packages
```
...
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo
python3 -m ruff check src/agents/agents.py
...
```make
test-unit:
	@python3 -m pytest tests/unit tests/eval -q

eval-prior-auth:
	@python3 scripts/eval_prior_auth.py --runs-dir data/cases
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && make test-unit && make eval-prior-auth
```
...
Run:
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && python3 -c "
import sys
try:
    import yaml
except ImportError:
...
```bash
cd /home/jokarash/dev/Azure-Healthcare-Marketplace-Demo && \
  python3 -m ruff check src/agents tests scripts && \
  python3 -m pytest tests/unit tests/eval -q && \
  python3 scripts/eval_prior_auth.py --runs-dir data/cases && \
...
1. `python3 -m pip install -r requirements-dev.txt && python3 -m pytest tests/unit tests/eval -q` passes.
